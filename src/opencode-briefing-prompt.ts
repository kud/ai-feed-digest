...
export function buildBriefingPrompt(items: EditionNarrativeItem[], timezone: string, targetReadingMinutes: number, language: string = "French"): string {
  ...
  return [
    ...
    "",
    "Strict additional constraints:",
    "- Do NOT prefix paragraphs with labels like 'Contexte:', 'Analyse:' or 'Overview:'. Provide pure prose.",
    "- Use concise feed names only once per paragraph; never repeat long taglines.",
    "- Never echo raw theme tokens verbatim; integrate them semantically.",
    "- Vary analytical verbs (évite répétitions: 'indique', 'souligne', 'accentue', 'montre').",
    "- If you cannot meet ≥2 distinct sources in a paragraph, merge that insight into another paragraph—never output a paragraph below threshold.",
    "- Avoid redundant phrases (e.g. 'signaux convergents' repeated).",
    "- Facts must be verifiable from provided sources; omit if not grounded.",
    "- Absolutely no invented numbers, dates, actors, quotes.",
