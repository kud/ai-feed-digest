...
export function buildBriefingPrompt(
  ...
  timezone: string,
  targetWords: { overview: number; analysis: number },
  ...
  return [
    ...
    "",
    `2. ANALYSIS (Analyse): 2-3 paragraphs, ~${targetWords.analysis} words total`,
    ...
    "- Add section labels like 'Analyse:' - the JSON structure handles that",
    "- Stack citations at paragraph end - weave them inline naturally",
    "",
    `Total target: ~${targetWords.overview + targetWords.analysis} words (overview: ${targetWords.overview}, analysis: ${targetWords.analysis}).`,
